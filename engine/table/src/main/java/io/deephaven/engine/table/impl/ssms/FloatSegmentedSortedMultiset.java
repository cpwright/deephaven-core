...
public final class FloatSegmentedSortedMultiset implements SegmentedSortedMultiSet<Float>, FloatVector {
    ...
    private int insertExistingIntoLeaf(WritableFloatChunk<? extends Values> valuesToInsert,
            ...
            long[] leafCounts, float maxInsert, boolean lastLeaf) {
        int rlpos = 0;
        float nextValue;
    ...
    private void maybeCompact(WritableFloatChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
            int ripos, int wipos) {
        if (wipos == ripos) {
            return;
        }
    ...
    private void insertExisting(WritableFloatChunk<? extends Values> valuesToInsert,
        ...
        if (leafCount == 1) {
            ...
            final int ripos = insertExistingIntoLeaf(valuesToInsert, counts, 0, wipos, size, directoryValues,
                    directoryCount, NULL_FLOAT, true);
    ...
    private void remove(RemoveContext removeContext, FloatChunk<? extends Values> valuesToRemove,
    ...
    private static void validateLeaf(float[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(lt(values[ii], values[ii + 1]), "lt(values[ii], values[ii + 1])", (Float) thisValue,
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(geq(getMinFloat(), destination.getMaxFloat()),
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(leq(getMaxFloat(), destination.getMinFloat()),

...
// ****** Edit DoubleSegmentedSortedArray and run "./gradlew replicateSegmentedSortedArray" to regenerate
//
// @formatter:off
 ...
 *
 * We must be totally ordered, which is accomplished by sorting on the double values, and then on the corresponding row
 * key.
 */
public final class DoubleReverseSegmentedSortedArray implements SegmentedSortedArray {
    ...
    private void copyToLeaf(int leafOffset, double[] leafValues, DoubleChunk<? extends Any> insertValues,
    ...
    private void makeSingletonLeaf(DoubleChunk<? extends Any> values, LongChunk<? extends RowKeys> rowKeys) {
    ...
    private void makeLeavesInitial(DoubleChunk<? extends Any> values, LongChunk<? extends RowKeys> rowKeys) {
    ...
    private void removeFromLeaf(int leafSize, double[] leafValues, DoubleChunk<? extends Any> removeValues,
    ...
    private static int lowerBound(DoubleChunk<? extends Any> valuesToSearch, LongChunk<? extends RowKeys> rowKeysToSearch,
            int lo, int hi, double searchValue, long searchKey) {
        while (lo < hi) {
    ...
    private static int upperBound(DoubleChunk<? extends Any> valuesToSearch, LongChunk<? extends RowKeys> rowKeysToSearch,
            int lo, int hi, double searchValue, long searchKey) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final double testValue = valuesToSearch.get(mid);
    ...
    private static int lowerBound(double[] valuesToSearch, long[] rowKeysToSearch, int lo, int hi, double searchValue,
            long searchKey) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
     ...
     */
    private static int bound(double[] valuesToSearch, long[] rowKeysToSearch, final double searchValue, long searchRowKey,
            int lo, int hi) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
            final double testValue = valuesToSearch[mid];
    ...
    public void remove(Chunk<? extends Any> valuesToRemove, LongChunk<? extends RowKeys> rowKeysToRemove) {
        remove(valuesToRemove.asDoubleChunk(), rowKeysToRemove);
    }
    ...
    public void removeAndGetPrior(Chunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> rowKeysToRemove,
        ...
        removeAndGetNextInternal(stampChunk.asDoubleChunk(), rowKeysToRemove, priorRedirections);
    }

    private void remove(DoubleChunk<? extends Any> valuesToRemove, LongChunk<? extends RowKeys> rowKeysToRemove) {
     ...
     */
    private void removeAndGetNextInternal(DoubleChunk<? extends Any> valuesToRemove,
    ...
    private void applyShift(DoubleChunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk,
            long shiftDelta) {
        validate();
    ...
    private void shiftLeaf(int leafSize, double[] leafValues, DoubleChunk<? extends Any> shiftValues, long[] leafRowKeys,
    ...
    private void applyShiftReverse(DoubleChunk<? extends Any> stampChunk, LongChunk<? extends RowKeys> keyChunk,
            long shiftDelta) {
        validate();
        final int shiftSize = stampChunk.size();
        if (shiftSize == 0) {
    ...
    private void shiftLeafReverse(int leafSize, double[] leafValues, DoubleChunk<? extends Any> shiftValues,
    ...
    private static void validateLeaf(double[] values, long[] rowKeys, int size) {
        if (!SEGMENTED_SORTED_ARRAY_VALIDATION) {
            return;
        }

...
public final class CharSegmentedSortedMultiset implements SegmentedSortedMultiSet<Character>, CharVector {
    ...
    private void distributeNewIntoLeaves(CharChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts,
    ...
    private void insertNewIntoLeaf(WritableCharChunk<? extends Values> valuesToInsert,
            WritableIntChunk<ChunkLengths> counts, int insertStart, int insertCount, int leafSize, char[] leafValues,
            long[] leafCounts) {
    ...
    private void copyRemainingValuesToLeaf(WritableCharChunk<? extends Values> valuesToInsert,
            WritableIntChunk<ChunkLengths> counts, int insertStart, char[] leafValues, long[] leafCounts, int ripos) {
     ...
     * @return the lowest index that is greater than or equal to valuesToSearch
     */
    private static int gallopBound(CharChunk<? extends Any> valuesToSearch, int lo, int hi, char searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
    ...
    private int removeFromLeaf(RemoveContext removeContext, CharChunk<? extends Values> valuesToRemove,
            IntChunk<ChunkLengths> counts, int ripos, int end, char[] leafValues, long[] leafCounts, MutableInt sz) {
        int rlpos = 0;
        int cl = -1;
        while (ripos < end) {
    ...
    private void validateInputs(CharChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts) {
        ...
        for (int ii = 1; ii < valuesToInsert.size(); ++ii) {
            ...
            Assert.assertion(CharComparisons.lt(prevValue, curValue), "CharComparisons.lt(prevValue, curValue)",
    ...
    private static void validateLeaf(char[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(lt(values[ii], values[ii + 1]), "lt(values[ii], values[ii + 1])", (Character) thisValue,
                    "values[ii]", (Character) nextValue, "values[ii + 1]", ii, "ii");
        }
        if (size > 0) {
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(geq(getMinChar(), destination.getMaxChar()),
                        "geq(getMinChar(), destination.getMaxChar())");
            }
        }
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(leq(getMaxChar(), destination.getMinChar()),
                        "leq(getMaxChar(), destination.getMinChar())");
            }
        }

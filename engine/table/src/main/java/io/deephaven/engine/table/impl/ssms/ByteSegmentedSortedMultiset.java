...
public final class ByteSegmentedSortedMultiset implements SegmentedSortedMultiSet<Byte>, ByteVector {
    ...
    private int insertExistingIntoLeaf(WritableByteChunk<? extends Values> valuesToInsert,
            WritableIntChunk<ChunkLengths> counts, int ripos, MutableInt wipos, int leafSize, byte[] leafValues,
            long[] leafCounts, byte maxInsert, boolean lastLeaf) {
        int rlpos = 0;
        byte nextValue;
        while (rlpos < leafSize && ripos < valuesToInsert.size()
            ...
            if (gt(leafValues[rlpos], nextValue)) {
                ...
                if (rlpos < leafSize) {
                    ...
                    valuesToInsert.copyFromTypedChunk((WritableByteChunk) valuesToInsert, ripos, wipos.get(),
                            lastInsert - ripos);
    ...
    private void distributeNewIntoLeaves(ByteChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts,
    ...
    private void insertNewIntoLeaf(WritableByteChunk<? extends Values> valuesToInsert,
            WritableIntChunk<ChunkLengths> counts, int insertStart, int insertCount, int leafSize, byte[] leafValues,
            long[] leafCounts) {
    ...
    private void copyRemainingValuesToLeaf(WritableByteChunk<? extends Values> valuesToInsert,
    ...
    private void maybeCompact(WritableByteChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts,
            int ripos, int wipos) {
        if (wipos == ripos) {
            return;
        }
    ...
    private void insertExisting(WritableByteChunk<? extends Values> valuesToInsert,
    ...
    private void packValuesIntoLeaves(ByteChunk<? extends Values> values, IntChunk<ChunkLengths> counts, int rpos,
     ...
     * @return the lowest index that is greater than or equal to valuesToSearch
     */
    private static int gallopBound(ByteChunk<? extends Any> valuesToSearch, int lo, int hi, byte searchValue) {
        while (lo < hi) {
            final int mid = (lo + hi) >>> 1;
    ...
    private void remove(RemoveContext removeContext, ByteChunk<? extends Values> valuesToRemove,
    ...
    private int removeFromLeaf(RemoveContext removeContext, ByteChunk<? extends Values> valuesToRemove,
            IntChunk<ChunkLengths> counts, int ripos, int end, byte[] leafValues, long[] leafCounts, MutableInt sz) {
        int rlpos = 0;
        int cl = -1;
        while (ripos < end) {
    ...
    private void validateInputs(ByteChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts) {
        ...
        for (int ii = 1; ii < valuesToInsert.size(); ++ii) {
            ...
            Assert.assertion(ByteComparisons.lt(prevValue, curValue), "ByteComparisons.lt(prevValue, curValue)",
    ...
    private static void validateLeaf(byte[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(lt(values[ii], values[ii + 1]), "lt(values[ii], values[ii + 1])", (Byte) thisValue,
                    "values[ii]", (Byte) nextValue, "values[ii + 1]", ii, "ii");
        }
        if (size > 0) {
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(geq(getMinByte(), destination.getMaxByte()),
                        "geq(getMinByte(), destination.getMaxByte())");
            }
        }
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(leq(getMaxByte(), destination.getMinByte()),
                        "leq(getMaxByte(), destination.getMinByte())");
            }
        }
    ...
    public ByteVector subVector(long fromIndexInclusive, long toIndexExclusive) {
        return new ByteVectorDirect(keyArray(fromIndexInclusive, toIndexExclusive));
    }

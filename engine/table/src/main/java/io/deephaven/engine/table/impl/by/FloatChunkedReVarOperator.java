...
class FloatChunkedReVarOperator implements IterativeChunkedAggregationOperator {
    ...
    FloatChunkedReVarOperator(String name, final boolean std, DoubleChunkedSumOperator sumSum,
            DoubleChunkedSumOperator sum2Sum, LongChunkedSumOperator nncSum, LongChunkedSumOperator nanSum,
            LongChunkedSumOperator picSum, LongChunkedSumOperator nicSum) {
        this.name = name;
        this.std = std;
        this.sumSum = sumSum;
    ...
    private void updateResult(ReVarContext reVarContext, RowSequence destinationOk,
        ...
        final DoubleChunk<Values> sum2SumChunk =
                sum2Sum.getChunk(reVarContext.sum2Context, destinationOk).asDoubleChunk();
        final LongChunk<? extends Values> nncSumChunk =
                nncSum.getChunk(reVarContext.nncContext, destinationOk).asLongChunk();
        final LongChunk<? extends Values> nanSumChunk =
                nanSum.getChunk(reVarContext.nanContext, destinationOk).asLongChunk();
        final LongChunk<? extends Values> picSumChunk =
                picSum.getChunk(reVarContext.picContext, destinationOk).asLongChunk();
        ...
        for (int ii = 0; ii < size; ++ii) {
            final boolean changed =
                    updateResult(reVarContext.keyIndices.get(ii), nncSumChunk.get(ii), nanSumChunk.get(ii),
                            picSumChunk.get(ii), nicSumChunk.get(ii), sumSumChunk.get(ii), sum2SumChunk.get(ii));

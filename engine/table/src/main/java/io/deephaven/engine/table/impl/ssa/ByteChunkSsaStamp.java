...
public class ByteChunkSsaStamp implements ChunkSsaStamp {
    ...
    public void processEntry(Chunk<Values> leftStampValues, Chunk<RowKeys> leftStampKeys, SegmentedSortedArray ssa,
        ...
        processEntry(leftStampValues.asByteChunk(), leftStampKeys, (ByteSegmentedSortedArray) ssa, rightKeysForLeft,
                disallowExactMatch);
    }

    private static void processEntry(ByteChunk<Values> leftStampValues, Chunk<RowKeys> leftStampKeys,
            ByteSegmentedSortedArray ssa, WritableLongChunk<RowKeys> rightKeysForLeft, boolean disallowExactMatch) {
    ...
    public void processRemovals(Chunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
        ...
        processRemovals(leftStampValues.asByteChunk(), leftStampKeys, rightStampChunk.asByteChunk(), rightKeys,
                priorRedirections, rowRedirection, modifiedBuilder, disallowExactMatch);
    }

    private static void processRemovals(ByteChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
            ByteChunk<? extends Values> rightStampChunk, LongChunk<RowKeys> rightKeys,
    ...
    public void processInsertion(Chunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
        ...
        processInsertion(leftStampValues.asByteChunk(), leftStampKeys, rightStampChunk.asByteChunk(), rightKeys,
                nextRightValue.asByteChunk(), rowRedirection, modifiedBuilder, endsWithLastValue, disallowExactMatch);
    }

    private static void processInsertion(ByteChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
            ByteChunk<? extends Values> rightStampChunk, LongChunk<RowKeys> rightKeys, ByteChunk<Values> nextRightValue,
            WritableRowRedirection rowRedirection, RowSetBuilderRandom modifiedBuilder, boolean endsWithLastValue,
            boolean disallowExactMatch) {
    ...
    public int findModified(int first, Chunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
        ...
        return findModified(first, leftStampValues.asByteChunk(), leftStampKeys, rowRedirection,
                rightStampChunk.asByteChunk(), rightStampIndices, modifiedBuilder, disallowExactMatch);
    }

    private static int findModified(int leftLowIdx, ByteChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
            RowRedirection rowRedirection, ByteChunk<? extends Values> rightStampChunk,
            LongChunk<RowKeys> rightStampIndices, RowSetBuilderRandom modifiedBuilder, boolean disallowExactMatch) {
        try (final SizedLongChunk<RowKeys> modifiedKeys = new SizedLongChunk<>()) {
            // the modified left rows are gathered and sorted, so the builder receives them in row key order
            int capacity = Math.max(1, Math.min(rightStampChunk.size(), leftStampKeys.size()));
    ...
    public void applyShift(Chunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
        ...
        applyShift(leftStampValues.asByteChunk(), leftStampKeys, rightStampChunk.asByteChunk(), rightStampKeys,
                shiftDelta, rowRedirection, disallowExactMatch);
    }

    private void applyShift(ByteChunk<Values> leftStampValues, LongChunk<RowKeys> leftStampKeys,
            ByteChunk<? extends Values> rightStampChunk, LongChunk<RowKeys> rightStampKeys, long shiftDelta,
            WritableRowRedirection rowRedirection, boolean disallowExactMatch) {
        int leftLowIdx = 0;
        for (int ii = 0; ii < rightStampChunk.size(); ++ii) {
            final byte rightStampValue = rightStampChunk.get(ii);
    ...
    private static int findFirstResponsiveLeft(int leftLowIdx, ByteChunk<Values> leftStampValues,
            boolean disallowExactMatch, byte rightStampValue) {

 ...
 */
public class FloatSparseArraySource extends SparseArrayColumnSource<Float>
    ...
    public void setNull(long key) {
        ...
        if (blocks2[indexWithinBlock] == NULL_FLOAT) {
            return;
        }
        ...
        blocks2[indexWithinBlock] = NULL_FLOAT;
    }
    // endregion setNull
    ...
    public void startTrackingPrevValues() {
        ...
        prevFlusher = new UpdateCommitter<>(this, updateGraph, FloatSparseArraySource::commitUpdates);
    }
    ...
    /* CONVERTER */) {
        ...
        for (int ii = 0; ii < keys.size();) {
            ...
            while (ii <= lastII) {
                ...
                final float[] blockToUse =
                        (prevInUse != null && (prevInUse[indexWithinInUse] & maskWithinInUse) != 0) ? prevBlock : block;
                // region conversion
                chunk.set(ii++, blockToUse == null ? NULL_FLOAT : blockToUse[indexWithinBlock]);
                // endregion conversion
            }
        }
        dest.setSize(keys.size());
    ...
    /* CONVERTER */) {
        ...
        for (int ii = 0; ii < keys.size();) {
            ...
            final float[] prevBlock = trackPrevious ? ensurePrevBlock(firstKey, block0, block1, block2) : null;
    ...
    // region getChunk
    @Override
    public FloatChunk<Values> getChunk(@NotNull final GetContext context, @NotNull final RowSequence rowSequence) {
        if (rowSequence.isEmpty()) {
    ...
    // region getPrevChunk
    @Override
    public FloatChunk<Values> getPrevChunk(@NotNull final GetContext context, @NotNull final RowSequence rowSequence) {
        if (prevFlusher == null) {

 ...
 */
public class ByteChunkedSumOperator implements IterativeChunkedAggregationOperator, ChunkSource<Values> {
    ...
    ByteChunkedSumOperator(boolean absolute, String name) {
    ...
    public void modifyChunk(BucketedContext context, Chunk<? extends Values> previousValues,
            ...
            WritableBooleanChunk<Values> stateModified) {
        final ByteChunk<? extends Values> newAsByteChunk = newValues.asByteChunk();
        final ByteChunk<? extends Values> oldAsByteChunk = previousValues.asByteChunk();
        for (int ii = 0; ii < startPositions.size(); ++ii) {
            final int startPosition = startPositions.get(ii);
            final long destination = destinations.get(startPosition);
            stateModified.set(ii,
                    modifyChunk(oldAsByteChunk, newAsByteChunk, destination, startPosition, length.get(ii)));
        }
    }
    ...
    public boolean modifyChunk(SingletonContext context, int chunkSize, Chunk<? extends Values> previousValues,
        ...
        return modifyChunk(previousValues.asByteChunk(), newValues.asByteChunk(), destination, 0, newValues.size());
    }

    private boolean modifyChunk(ByteChunk<? extends Values> oldValues, ByteChunk<? extends Values> newValues,
    ...
    private long doSum(int chunkStart, int chunkSize, MutableInt nonNullCount,
            ByteChunk<? extends Values> asByteChunk) {
        if (absolute) {
            return SumByteChunk.sumByteChunkAbs(asByteChunk, chunkStart, chunkSize, nonNullCount);
        } else {
            return SumByteChunk.sumByteChunk(asByteChunk, chunkStart, chunkSize, nonNullCount);
        }
    }

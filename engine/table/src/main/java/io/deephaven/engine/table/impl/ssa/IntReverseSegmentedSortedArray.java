...
public final class IntReverseSegmentedSortedArray implements SegmentedSortedArray {
    ...
    public <T extends Any> int insertAndGetNextValue(Chunk<T> valuesToInsert,
        ...
        final IntChunk<T> insertChunk = valuesToInsert.asIntChunk();
        if (leafCount == 0
            ...
            nextValue.asWritableIntChunk().copyFromTypedChunk(insertChunk, 1, 0, insertSize - 1);
            return insertSize - 1;
        }
        insert(insertChunk, rowKeysToInsert, WritableIntChunk.upcast(nextValue.asWritableIntChunk()));
     ...
     */
    private void insert(IntChunk<? extends Any> valuesToInsert, LongChunk<? extends RowKeys> rowKeysToInsert,
            @Nullable WritableIntChunk<Any> nextValues) {
        final int insertSize = valuesToInsert.size();
        validate();
    ...
    private void recordNextLeafFirst(int leaf, LongChunk<? extends RowKeys> rowKeysToInsert, int insertPosition,
            WritableIntChunk<Any> nextValues) {
    ...
    private void distributeValues(int targetSize, int startingLeaf, int distributionSlots,
            ...
            @Nullable WritableIntChunk<Any> nextValues, int nextOffset) {
        ...
        final int[] sourceValues = leafValues[startingLeaf];
        ...
        for (int workingSlot = startingLeaf + distributionSlots - 1; workingSlot >= startingLeaf; workingSlot--) {
            ...
            final int[] slotValues = leafValues[workingSlot];
            ...
            for (int wpos = leafSize - 1; wpos >= 0; wpos--) {
                ...
                final int vall = sourceValues[rposl];

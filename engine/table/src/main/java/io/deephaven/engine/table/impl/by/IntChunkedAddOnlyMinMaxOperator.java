...
class IntChunkedAddOnlyMinMaxOperator implements IterativeChunkedAggregationOperator {
    ...
    private static int min(IntChunk<?> values, MutableInt chunkNonNull, int chunkStart, int chunkEnd) {
        int nonNull = 0;
    ...
    private static int max(IntChunk<?> values, MutableInt chunkNonNull, int chunkStart, int chunkEnd) {
        int nonNull = 0;
        int value = QueryConstants.NULL_INT;
        for (int ii = chunkStart; ii < chunkEnd; ++ii) {
            final int candidate = values.get(ii);

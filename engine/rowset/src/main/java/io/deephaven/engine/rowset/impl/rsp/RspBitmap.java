...
public class RspBitmap extends RspArray<RspBitmap> implements OrderedLongSet {
    ...
    public void addValuesUnsafeNoWriteCheck(final LongChunk<OrderedRowKeys> values, final int offset,
        ...
        // The full block span most recently produced by a block filling up, which later blocks may extend: its first
        // and last block keys and its length, and where it lives -- an array index, or the last pending insert.
        long lastFullKey = -1;
        long lastFullLastKey = -1;
        long lastFullFlen = 0;
        int lastFullIdx = -1;
        boolean lastFullPending = false;
        int spanIndex = 0;
